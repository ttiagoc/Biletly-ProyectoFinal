...
describe("NFTTicketing", function () {
  ...
  describe("Minting NFTs", function () {
    ...
    it("Should track each minted NFT", async function () {
      ...
      await nft
        .connect(deployer)
        .mint(addr1.address, URI, "Segundo NFT", evento1);
    ...
    it("Should revert because caller is not the owner", async function () {
      // HAY ONLYOWNER
      await expect(
        nft.connect(addr1).mint(addr1.address, URI, "Nuevo NFT", evento1)
      ).to.be.rejected; // solo el owner puede crear nfts
    });
  ...
  describe("Minting NFTs with royalties", function () {
    beforeEach(async function () {
      await nft
        .connect(deployer)
        .mint(addr1.address, URI, "Nuevo NFT", evento1);
      await nft
        .connect(deployer)
        .mint(addr2.address, URI, "Segundo NFT", evento1);
    ...
    it("Mint two tokens and set two different royalties", async function () {
      ...
      expect(token1Royalty[0][0]).to.equal(deployer.address); // token0Royalty[0][0] toma la cuenta que recibe el porcentaje de reventa
  ...
  describe("Using Tickets", function () {
    ...
    it("Should stablish ticket as used", async function () {
      ...
      await expect(
        nft.connect(addr1).useTicket(1, idEvento)
      ).to.be.rejectedWith("ERC721: Ticket has already been used");
    });
  });
});
