...
describe("NFTMarketplace", function (){
    ...
    describe("Minting NFTs", function (){
        ...
        it("Should track each minted NFT", async function (){
            // HAY ONLYOWNER
            await expect(nft.connect(addr1).mint(URI, "Nuevo NFT", toWei(10), evento1)).to.be.reverted; // solo el owner puede crear nfts

            await nft.connect(deployer).mint(URI, "Primer NFT", toWei(1), evento1);
            expect(await nft.tokenCount()).to.equal(1);
            expect(await nft.balanceOf(deployer.address)).to.equal(1);
            expect(await nft.tokenURI(1)).to.equal(URI);

            await nft.connect(deployer).mint(URI, "Segundo NFT", toWei(2), evento1);
    ...
    describe("Making marketplace items", function (){
        ...
        it("Should track newly created NFTs", async function(){
            await expect(nft.connect(deployer).mint(URI, "Nuevo NFT", toWei(price), evento1))
            .to.emit(nft, "Offered")
            .withArgs(
                1,                
                toWei(price),
    ...
    describe("Selling Tickets", function (){
        ...
        beforeEach(async function(){
            await nft.connect(deployer).mint(URI, "Nuevo NFT", toWei(price), evento1);
        ...
        it ("Should update item as sold, pay seller, transfer to buyer...", async function(){
            ...
            await nft.connect(deployer).transferFrom(await nft.getOwner(1), addr1.address, 1 )           
